...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.submitForm = function() {
        ...
        setTimeout(async () => {
            try {
                ...
                subtitle.innerText = "Nous n'avons pas pu générer votre séance. Veuillez réessayer.";
    ...
    function generateWorkout(profile, allExos) {
        ...
        const allowedDifficulties = difficultyMap[profile.experience] || ['debutant'];

        const equipmentMap = {
            'Aucun': ['poids_du_corps', 'tapis', 'aucun', 'mur', 'chaise', 'marche_escalier'],
            'Basique': ['poids_du_corps', 'tapis', 'aucun', 'mur', 'chaise', 'marche_escalier', 'elastique', 'kettlebell', 'halteres', 'banc', 'banc_solide', 'box'],
            'Complet': null
        };
        ...
        const userSports = Array.isArray(profile.sports) ? profile.sports : (profile.sports ? [profile.sports] : []);

        const filteredExos = allExos.filter(exo => {
            const difficultyMatch = allowedDifficulties.includes(exo.difficulty);
            const equipmentMatch = (profile.materiel === 'Complet') || exo.equipment.every(eq => allowedEquipment.includes(eq));
            const typeMatch = allowedTypes.includes(exo.type);
            return difficultyMatch && equipmentMatch && typeMatch;
        });

        const sportValueMap = { 'Crossfit': 'crossfit', 'Cyclisme': 'cyclisme', 'Musculation': 'musculation', 'SportsAquatiques': 'sports_aquatique', 'SportsRelaxation': 'sport_relaxation', 'Randonnees': 'randonnee', 'Running': 'running', 'SportsCollectifs': 'sport_collectif', 'SportsRaquettes': 'sports_raquete' };
        
        const scoredExos = filteredExos.map(exo => {
            let score = 0;
            if (userSports.length > 0) {
                userSports.forEach(sport => {
                    const jsonSport = sportValueMap[sport];
                    if (jsonSport && exo.sport_category.includes(jsonSport)) score++;
                });
            }
    ...
    function displayWorkout(workout, profile) {
        ...
        if (!workout || workout.length === 0) {
            ...
            subtitle.innerText = "Aucun exercice ne correspond parfaitement à vos critères. Essayez une autre sélection.";
            subtitle.style.display = 'block';
            return;
        }
        ...
        const repsMap = {
            ...
            'Intermediaire': { sets: '3', reps: '15 Répétitions', time: '45 sec' },
        ...
        let workoutHtml = '';
        workout.forEach(exo => {
            const isTimeBased = ['etirement', 'relaxation', 'cardio'].includes(exo.type) || exo.title.toLowerCase().includes('planche') || exo.title.toLowerCase().includes('chaise');
